...
import me.vexon.client.renderer.Renderer3D;
...
import me.vexon.client.utils.network.MeteorExecutor;
...
public class HoleTunnelStairsESP extends Module {
    ...
    public HoleTunnelStairsESP() {
        super(Category.VEXON_ESP, "Hole Tunnel Stairs ESP", "Finds and highlights holes, tunnels and staircases.");
    }
    ...
    private void renderHoles(Renderer3D r) {
        ...
        for (Box b : holes)
            r.box(b, side, line, shapeMode.get(), 0);
    }
    ...
    private void searchChunk(long chunkKey) {
        if (mc.world == null)
            return;
        if (!loadedKeys.contains(chunkKey))
            return;
                        ...
                        for (int y = 0; y < 16; y++) {
                            ...
                            switch (detectionMode.get()) {
                                case ALL -> {
                                    ...
                                    if (diagonals.get())
                                        checkDiagonalTunnel(pos, chunkKey);
    ...
    private void checkHole(BlockPos pos, long chunkKey) {
        ...
        if (holes.contains(hole))
            return;
        if (holes.stream().anyMatch(existing -> existing.intersects(hole)))
            return;
    ...
    private void checkTunnel(BlockPos pos, long chunkKey) {
        for (Direction dir : DIRECTIONS) {
            ...
            if (isTunnelSection(cur, dir))
                start = cur.toImmutable();
            ...
            if (start == null || end == null)
                continue;
            if (steps < minTunnelLength.get())
                continue;
            if (maxHeightFound < minTunnelHeight.get() || maxHeightFound > maxTunnelHeight.get())
                continue;
            ...
            if (tunnels.contains(tunnel))
                continue;
            if (tunnels.stream().anyMatch(e -> e.intersects(tunnel)))
                continue;
    ...
    private boolean isTunnelSection(BlockPos pos, Direction dir) {
        ...
        if (height < minTunnelHeight.get() || height > maxTunnelHeight.get())
            return false;

        if (isPassableBlock(pos.down()) || isPassableBlock(pos.up(height)))
            return false;
        ...
        for (Direction p : perp) {
            for (int i = 0; i < height; i++) {
                if (isPassableBlock(pos.up(i).offset(p)))
                    return false;
            }
        }
        ...
        for (Direction dir : DIRECTIONS) {
            for (int step = minDiagonalWidth.get() - 1; step < maxDiagonalWidth.get(); step++) {
                ...
                while (isDiagonalTunnelSection(cur, checking)) {
                    ...
                    Box b = new Box(cur.getX(), cur.getY(), cur.getZ(), cur.getX() + 1, cur.getY() + height,
                            cur.getZ() + 1);
                    if (potential.stream().noneMatch(e -> e.intersects(b)) && !potential.contains(b))
                        potential.add(b);
                ...
                if (minDiagonalWidth.get() <= 0)
                    continue;
                if (steps / minDiagonalWidth.get() < minDiagonalLength.get())
                    continue;
    ...
    private boolean isDiagonalTunnelSection(BlockPos pos, Direction dir) {
        ...
        for (int i = 0; i < height; i++) {
            if (isPassableBlock(pos.up(i).offset(dir)))
                return false;
        }
    ...
    private int getTunnelHeight(BlockPos pos) {
        int h = 0;
        while (h < maxTunnelHeight.get() && isPassableBlock(pos.up(h)))
            h++;
        return h;
    }
    ...
    private void checkStaircase(BlockPos pos, long chunkKey) {
        for (Direction dir : DIRECTIONS) {
            ...
            if (steps < minStaircaseLength.get())
                continue;
    ...
    private int getStaircaseHeight(BlockPos pos) {
        int h = 0;
        while (h < maxStaircaseHeight.get() && isPassableBlock(pos.up(h)))
            h++;
        return h;
    }
    ...
    private boolean isStaircaseSection(BlockPos pos, Direction dir) {
        ...
        if (height < minStaircaseHeight.get() || height > maxStaircaseHeight.get())
            return false;
