...
public class CactusChunkScanner implements ChunkScanner<CactusChunkData> {
    private final World world;
    private final IntSupplier minHeight;

    public CactusChunkScanner(World world, IntSupplier minHeight) {
        this.world = world;
        this.minHeight = minHeight;
    }

    @Override
    public CactusChunkData scan(WorldChunk chunk) {
        ...
        for (int x = xStart; x < xStart + 16; x++) {
            for (int z = zStart; z < zStart + 16; z++) {
                for (int y = yMin; y < yMaxExclusive; y++) {
                    mutable.set(x, y, z);
                    if (!chunk.getBlockState(mutable).isOf(Blocks.CACTUS))
                        continue;

                    below.set(x, y - 1, z);
                    if (world.getBlockState(below).isOf(Blocks.CACTUS))
                        continue;
        ...
        return new CactusChunkData(qualifyingColumns, maxHeight);
    }
}
