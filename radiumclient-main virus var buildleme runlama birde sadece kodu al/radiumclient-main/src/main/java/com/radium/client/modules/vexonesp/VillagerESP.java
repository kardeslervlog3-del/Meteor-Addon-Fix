...
public class VillagerESP extends Module {
    ...
    private final Setting<SettingColor> zombieVillagerTracerColor = sgRender.add(new ColorSetting.Builder()
            ...
            .visible(() -> showTracers.get() && (detectionMode.get() == DetectionMode.ZombieVillagers
                    || detectionMode.get() == DetectionMode.Both))
            .build());
    ...
    public VillagerESP() {
        super(Category.VEXON_ESP, "Villager ESP", "Detects villagers and zombie villagers in the world.");
    }
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        if (mode == DetectionMode.Villagers || mode == DetectionMode.Both) {
            for (VillagerEntity v : mc.world.getEntitiesByType(TypeFilter.instanceOf(VillagerEntity.class), searchBox,
                    e -> true)) {
                villagerCount++;
                current.add(v.getId());

                if (tracers) {
                    me.vexon.client.utils.Utils.set(pos, v, event.tickDelta);
                    double x = pos.x;
                    double y = pos.y;
                    double z = pos.z;
                    y += v.getBoundingBox().getLengthY() / 2.0;
                    event.renderer.line(cameraPos.x, cameraPos.y, cameraPos.z, x, y, z, villColor);
                }
            }
        }

        if (mode == DetectionMode.ZombieVillagers || mode == DetectionMode.Both) {
            for (ZombieVillagerEntity z : mc.world.getEntitiesByType(TypeFilter.instanceOf(ZombieVillagerEntity.class),
                    searchBox, e -> true)) {
                zombieCount++;
                current.add(z.getId());

                if (tracers) {
                    me.vexon.client.utils.Utils.set(pos, z, event.tickDelta);
                    double x = pos.x;
                    double y = pos.y;
                    double zz = pos.z;
                    y += z.getBoundingBox().getLengthY() / 2.0;
                    event.renderer.line(cameraPos.x, cameraPos.y, cameraPos.z, x, y, zz, zomColor);
                }
            }
        }
    ...
    private String buildDetectionMessage(int villagerCount, int zombieVillagerCount) {
        ...
        if (villagerCount > 0)
            return String.format("%d villagers detected!", villagerCount);
        if (zombieVillagerCount > 0)
            return String.format("%d zombie villagers detected!", zombieVillagerCount);
        return "Villagers detected!";
    }
