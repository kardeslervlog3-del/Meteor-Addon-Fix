...
public class ChunkFinderRuntime<T> {
    ...
    public ChunkFinderRuntime(MinecraftClient mc, ChunkScanner<T> scanner, ChunkFlagEvaluator<T> evaluator,
            BiConsumer<ChunkPos, T> onFirstFlag, int threadPoolSize) {
        this.mc = mc;
    ...
    public ChunkFinderRuntime(MinecraftClient mc, ChunkScanner<T> scanner,
            ChunkFlagContextEvaluator<T> contextEvaluator, BiConsumer<ChunkPos, T> onFirstFlag, int threadPoolSize,
    ...
    public void tickCleanup(long nowMs, long cleanupIntervalMs, ChunkPos playerChunk, int chunkBuffer) {
        ...
        flaggedChunks.removeIf(
                cp -> Math.abs(cp.x - playerChunk.x) > chunkBuffer || Math.abs(cp.z - playerChunk.z) > chunkBuffer);
        chunkData.keySet().removeIf(
                cp -> Math.abs(cp.x - playerChunk.x) > chunkBuffer || Math.abs(cp.z - playerChunk.z) > chunkBuffer);
        lastCleanupMs = nowMs;
    }
    ...
    private void evaluateSingle(ChunkPos chunkPos, T data) {
        ...
        if (contextEvaluator != null)
            shouldFlag = contextEvaluator.shouldFlag(chunkPos, data, chunkData);
        else
            shouldFlag = evaluator != null && evaluator.shouldFlag(data);

        if (shouldFlag) {
            if (flaggedChunks.add(chunkPos))
                onFirstFlag.accept(chunkPos, data);
        } else {
