...
import me.vexon.client.VexonMain;
import me.vexon.client.systems.modules.Categories;
import me.vexon.client.events.render.Render2DEvent;
import me.vexon.client.renderer.Renderer2D;
import me.vexon.client.renderer.text.TextRenderer;
...
public class RegionMap extends Module {
    ...
    public RegionMap() {
        super(Category.VEXON_ESP, "Region Map", "Vexon region map with directional indicator.");
    }
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        Renderer2D.COLOR.quad(x - 4, y - 4, totalGridSize + 8, totalGridSize + 8, bgColor.get());
        // outline (4 thin quads around the background)
        int ow = 1; // outline width
        Color oc = new Color(outlineColor.get());
        Renderer2D.COLOR.quad(x - 4, y - 4, totalGridSize + 8, ow, oc); // top
        Renderer2D.COLOR.quad(x - 4, y + totalGridSize + 4 - ow, totalGridSize + 8, ow, oc); // bottom
        Renderer2D.COLOR.quad(x - 4, y - 4, ow, totalGridSize + 8, oc); // left
        Renderer2D.COLOR.quad(x + totalGridSize + 4 - ow, y - 4, ow, totalGridSize + 8, oc); // right
        ...
        renderPlayerPin(x, y, size, gap);
        renderInternalInfo(x, y + totalGridSize + 10);
    }

    private void renderPlayerPin(int x, int y, int size, int gap) {
        ...
        renderDirectionalIndicator(centerX, centerY, rotationAngle, playerIndicatorColor.get(), arrowSize);
    }

    private void renderDirectionalIndicator(int centerX, int centerY, double angle,
            SettingColor color, int arrowSize) {
        try {
            Renderer2D.COLOR.begin();
    ...
    private void renderInternalInfo(int x, int y) {
        TextRenderer.get().begin(1.1, false, true);

        Color themeColor = VexonMain.ADDON.color;
        String posStr = String.format("Pos: %d, %d", (int) mc.player.getX(), (int) mc.player.getZ());
        TextRenderer.get().render(posStr, x, y, themeColor, false);
        ...
        TextRenderer.get().render("Region: " + regId + " (" + regName + ")", x, y + 12, themeColor, false);
    ...
    private static class MapDataManager {
        ...
        public MapDataManager() {
            int[][] layout = new int[][] {
                    ...
                    { 89, 0 }, { 26, 1 }, { 27, 0 }, { 28, 0 }, { 29, 0 }, { 30, 0 }, { 59, 1 }, { 32, 0 }, { 98, 0 },
